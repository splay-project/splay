...
local function reply(s, data, ip, port)
	...
	if data.type == "call" then
		...
		if length > 8192 then
			...
			-- we was optimistic, we need to reencode now...
			reply_s = enc.encode(data)
...
function server(port)
	...
	-- TODO
	-- If (unlucky..) the port is the same that the default socket has choosed, we
	-- should:
	-- - close the default socket
	-- - set server_run = false
	-- - retry binding this port
	-- - the default socket will be started again on another port
