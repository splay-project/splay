...
function send_get(port, type_of_transaction, key)
	...
	local max_value = 0
	local max_value_vc = nil
	for i2,v2 in ipairs(answer) do
		if v2.value > max_value then
			max_value = v2.value
			max_value_vc = v2.vector_clock
		end
	end
	print("key: "..key..", value: "..max_value..", vector_clock")
	for i2,v2 in pairs(max_value_vc) do
		print("", i2, v2)
	end
