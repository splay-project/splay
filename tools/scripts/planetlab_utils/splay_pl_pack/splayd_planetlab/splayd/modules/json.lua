...
--base.print(startPos, "curchar: ", curChar, base.string.sub(s, startPos, startPos + 40))
  -- Object
  if curChar=='{' then
...
--base.print("array", object, startPos)
...
function decode_scanConstant(s, startPos)
  ...
  for i,k in base.pairs(constNames) do
		--base.print ("[" .. string.sub(s,startPos, startPos + string.len(k) -1) .."]", k)
    if string.sub(s,startPos, startPos + string.len(k) -1 )==k then
--base.print(consts[k], startPos + string.len(k))
...
function replace_unicode(s)
	...
	while i <= string.len(s) do
		...
		if escape == false and c == [[\]] then
			...
			if escape == true and c == "u" then
				o[o_i] = json_uchar_to_chars(string.sub(s, i + 1, i + 4))
				o_i = o_i + 1
				i = i + 4
			else
				if escape == true then
	...
	return base.table.concat(o)
end

function replace_unicode_old(s)
	out = ''
	escape = false
	i = 1
	while i <= string.len(s) do
		...
		if escape == false and c == [[\]] then
			...
			if escape == true and c == "u" then
				out = out..json_uchar_to_chars(string.sub(s, i + 1, i + 4))
				i = i + 4
			else
				if escape == true then
					out = out..[[\]]
...
function hex_char_to_num(c)
	c = string.byte(string.lower(c))
	if c >= 97 and c <= 102 then
		c = c - 87
	else
		base.assert(c >= 48 and c <= 58, "not valid hex char")
		c = c - 48
	end
	return c
end
...
function json_uchar_to_chars(s)
