...
wget "http://splay2.unineuchatel.ch/splayd_ng_planetlab.tar.gz?${rd}"
mv splayd_ng_planetlab.tar.gz?${rd} splayd_planetlab.tar.gz
...
echo "echo 'su -l $user -c /home/$user/leo/run.sh'  >> /etc/rc.local" >> local
sudo bash local
rm local
#fi
