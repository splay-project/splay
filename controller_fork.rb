...
for i in (0...Config::NumLogd)
	fork do
		$db = DBUtils.get_new
		LogdServer.new(Config::LogdPort + i).run.join
	end
end

for i in (0...Config::NumSplayd)
	fork do
		$db = DBUtils.get_new
		$dbt = DBUtils.get_new
		SplaydServer.new(Config::SplaydPort + i).run.join
	end
end
