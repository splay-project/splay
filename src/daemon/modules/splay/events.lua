...
local function single_thread(th)
	...
	_M.l_o:debug("single_thread, before adding co:", #new_threads,new_threads)
	new_threads[#new_threads + 1] = co
	_M.l_o:debug("single_thread, after  adding co:", #new_threads,new_threads)	
	_M.l_o:debug("#new_threads:",#new_threads,"threads_ref["..name.."]:",co, "STATUS:", coroutine.status(co))
	_M.l_o:debug("returning "..name.." from single_thread(th)")
	return name
end
