...
require File.expand_path(File.join(File.dirname(__FILE__), 'lua-merger.rb'))
#Main class
class Ctrl_api
  #function get_log: triggered when a "GET LOG" message is received, 
  #returns the corresponding log file as a string
  ...
  def get_job_code(job_id, session_id)
  	...
  	#checks the validity of the session ID and stores the returning value in the variable user
  	user = check_session_id(session_id)
  	#check_session_id returns false if the session ID is not valid;
        #if user is not false (the session ID is valid)
  	if (user) then
          #user_id is taken from the field 'id' from variable user
          user_id = user['id']
...
if SplayControllerConfig::AllowNativeLibs
  ...
  def submit_lib(lib_name, lib_version, lib_os, lib_arch, lib_sha1, lib_code, session_id,
    ...
    #if we are here there is no lib with the given hash, 
    ...
    if admin then
      ...
      #$db.do("SELECT * FROM libs WHERE lib_name='#{lib_name}' AND lib_os='#{lib_os}' AND lib_arch='#{lib_arch}'")
    	if existing_lib then
          ...
          $db.from(:lib).insert(:lib_name => lib_name, :lib_version => lib_version, :lib_os => lib_os, :lib_arch => lib_arch, :lib_sha1 => lib_sha1, :lib_blob => addslashes(lib_code))
    	  #$db.do("INSERT INTO libs SET lib_name='#{lib_name}', 
          #lib_version='#{lib_version}', lib_os='#{lib_os}', 
          #lib_arch='#{lib_arch}', lib_sha1='#{lib_sha1}', lib_blob='#{addslashes(lib_code)}'")
          ret['ok'] = true
  ...
  def list_libs(lib_name, session_id)
  	...
  	if user then
  		...
  		#TODO give a detailed presentation of the libs, and possibly the splayds they are deployed on
                  $db.from(:libs).where('lib_name = ?', lib_name).each do |a_lib|
#                      $db.do("SELECT * FROM libs WHERE lib_name='#{lib_name}'") do |a_lib|
                    lib = Hash.new
  	...
  	#list all libs in db
  		$db['select * from libs'].each do |a_lib|
                #do("SELECT * FROM libs") do |a_lib|
  			lib = Hash.new
  			lib['lib_name'] = a_lib[:lib_name]
  			lib['lib_version'] = a_lib[:lib_version]
  			lib['lib_os'] = a_lib[:lib_os]
  ...
  def remove_lib(lib_name, lib_version, lib_arch, lib_os, lib_sha1, session_id,
    ...
    if admin then
	  ...
	  #  end
          ##puts where_clause
          $db.from(:libs).where("lib_name = ? AND lib_version = ? AND lib_arch = ? AND lib_os = ? AND lib_sha1 = ?", lib_name, lib_version, lib_arch, lib_os, lib_sha1).delete
          #do("DELETE FROM libs #{where_clause}")
          ret['ok'] = true
...
def submit_job(name, description, code, lib_filename, lib_version, nb_splayds, churn_trace,
                 options, session_id, scheduled_at, strict, trace_alt, queue_timeout,
                 multiple_code_files, designated_splayds_string, splayds_as_job,  topology)
    ret = Hash.new
	...
	if code == "" then
      ...
      if designated_splayds_string != "" then
        ...
        designated_splayds_string.split(',').each { |splayd_id|
          $db.from(:job_designated_splayds).insert(:job_id => job_id, :splay_id => splayd_id)
      	  #q_jds = q_jds + "('#{job_id}','#{splayd_id}'),"
      	}
      	#q_jds = q_jds[0, q_jds.length - 1]
        #.do("INSERT INTO  (job_id, splayd_id) VALUES #{q_jds}")
      end
      
      #use the same splayds as another job
      if splayds_as_job != "" then
      	job_id = $db.from(:jobs).where('ref = ?', ref)[:id]
        #.do("SELECT id FROM jobs WHERE ref='#{ref}'")
...
#$db.do "INSERT INTO job_designated_splayds (job_id,other_job_status) VALUES ('#{job_id}','#{other_job_status}')"
	  ...
	  other_job_nb_splayds = $db.from(:nb_splayds).where('id = ?', other_job_id)[:nb_splayds]
          #$db.do("SELECT nb_splayds FROM jobs WHERE id='#{other_job_id}'")
	  $db.from(:jobs).where('id = ?', job_id).update(:nb_splayds => other_job_nb_splayds)
  ...
  #function get_job_details: triggered when a "GET JOB DETAILS" message is received,
  #returns the description, status and host list of the job
  ...
  def list_jobs(session_id)
    ...
    if (user) then
    	...
    	if (user['admin'] == 1) then
    		...
    		$db.from(:jobs).where('user_id = ?', user_id).each do |ms| 
    			job = Hash.new
    			job['id'] = ms[:id]
    			job['status'] = ms[:status]
    			job_list.push(job)
  ...
  #function start_session: triggered when a "START SESSION" message is received, triggers 
  #the granting of a token or session.
  ...
  def start_session(username, hashed_password)
    ...
    if user then
      ...
      if (hashed_password == hashed_password_from_db) then
        ...
        $db.from(:users).where('login = ?', username).update(:remember_token => remember_token, :remember_token_expires_at => remember_token_expires_at)
	ret['ok'] = true
  ...
  #function list_users: triggered when a "LIST USERS" message is received, returns a 
  # list of the users (only for administrators)
  ...
  def change_passwd(username, hashed_currentpassword, hashed_newpassword)
    ...
    if user then
      ...
      if (hashed_currentpassword == hashed_password_from_db) then
        $db.from(:users).where('login = ?', username).update(:crypted_password => hashed_newpassword)
        ret['ok'] = true
        return ret
      end
    end
  ...
  #function check_session_id: generic function that checks the validity of a session ID,
  ...
  def check_session_id(session_id)
    ...
    if user then
      ...
      expires_at_time_format = Time.local(expires_at.year, expires_at.month, expires_at.day,
        expires_at.hour, expires_at.min, expires_at.sec)
      time_now = Time.new()
      if (time_now < expires_at_time_format) then
    	...
    	user_id = user[:id]
        $db.from(:users).where('id = ?',user[:id]).update(:remember_token => nil,:remember_token_expires_at => nil)
    	return false
      end
    else
    	return false
