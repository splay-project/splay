...
function sim2(up, dp)
	local c = 0
	for _, id in pairs(up) do
		for _, did in pairs(dp) do
			if id == did then c = c + 1 end
		end
	end
	return c
end
...
function store(rfi)
	...
	if not docs[url].qds[query] then
		...
		docs[url].qds[query].dp = rfi.d.dp
		-- This one was considering that same query generates different snippets for
		-- a same document depending on the search engine. It's not our case.
...
-- qds: query => {count, max_rate, rate, rate_count, rate_queue*, snippet, dp}
			...
			local pop = 0
			for a, b in pairs(docs[url].qds) do
				pop = pop + b.count
			end
			...
			local o = {
				...
				n = sim2(msg.up, docs[url].qds[msg.q].dp),
				--pop = docs[url].qds[msg.q].count
				pop = pop
			}
			table.insert(r, o)
			i = i + 1
...
events.loop(function()
	...
	thread{rates_manager, rfi_rates_manager, log_bench}
